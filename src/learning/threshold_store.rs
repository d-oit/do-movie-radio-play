...
pub(crate) async fn get_threshold_recommendations(conn: &Connection) -> Result<ThresholdRecommendation> {
    let mut rows = conn
        .query(
            "SELECT
...
pub(crate) async fn get_latest_threshold(conn: &Connection) -> Result<Option<ThresholdHistoryEntry>> {
    let mut rows = conn
        .query(

...
| `crates/movie-radio-goap/src/narrate.rs` (now `narrate/mod.rs`) | Narration generator emitted content-free filler ("Stille."/"Pause.") for most gaps: `extract_context` never read the gap segment's own detected tags (only unrelated neighbouring gaps in the persisted, inversion-only timeline), and the filler phrases were double-weighted in the old hash-selected template pool | Grounded `generate_text` in the gap's own tags per German ARD/MDR audio-description + DCMP/W3C standards; added a runtime guardrail (`reject_banned_filler`) that substitutes a safe fallback if text ever collapses to a banned filler phrase; regression tests. See [ADR-128](adr/0128-audio-description-standards.md) (PR #345, issue #344) |
| `crates/movie-radio-pipeline/src/voice_clone.rs` | `voice samples` returned zero candidates on real movies: a dialogue span had to fit entirely within `[min_sample_seconds, 15s]`, so any continuous scene longer than 15s (the norm) was dropped outright | Tile every span into consecutive ≤15s clips instead of requiring the whole span to fit in one window; regression test (PR #343, issue #342) |

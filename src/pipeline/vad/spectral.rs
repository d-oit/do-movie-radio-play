...
impl VadEngine for SpectralVad {
    fn classify(&self, frames: &[Frame]) -> VadResult {
        ...
        for frame in frames {
            let likelihood = classify_spectral(frame, self.threshold);
...
fn classify_spectral(frame: &Frame, threshold: f32) -> f32 {
    ...
    let flatness_term = if frame.spectral_flatness > 0.4 {
        -0.20
    } else if frame.spectral_flatness > 0.3 {
        -0.10
    } else if frame.spectral_flatness < 0.15 {
        0.08
    } else {
        0.0
    };

    let entropy_term = if frame.spectral_entropy < 4.0 {
        0.20
    } else if frame.spectral_entropy < 5.0 {
        0.10
    } else if frame.spectral_entropy > 6.5 {
        -0.25
    } else if frame.spectral_entropy > 5.8 {
        -0.12
    } else {
        0.0
    };
    ...
    let voice_indicator = frame.zcr > 0.02
        && frame.zcr < 0.4
        && frame.centroid_hz > 200.0
        && frame.centroid_hz < 5000.0
        && frame.spectral_flatness < 0.5
        && frame.spectral_entropy < 6.0;

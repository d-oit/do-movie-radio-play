...
2. **Categorize issues**:
   ...
   - Real issues → Fix in code (e.g., run `cargo clippy`).
...
4. **Fix issues**: Batch fix patterns and verify with `bash scripts/quality_gate.sh`.
...
- [ ] Relying solely on local `codacy-analysis` without checking Cloud CLI.

...
use crate::review;
use crate::util;
use movie_radio_io::json::{read_json, read_timeline, write_json_pretty};
use movie_radio_learning::adaptive_thresholds;
use movie_radio_learning::database;
use movie_radio_pipeline::pipeline::{extract_timeline, prompts, tags};
use movie_radio_types::{Segment, SegmentKind};
use movie_radio_verification::verification::{
    verify_timeline, VerificationReport, VerificationStatus,
};
...
pub fn handle_validate(
    ...
    match (truth_json, subtitles, dataset_manifest) {
        (Some(truth_json), None, None) => {
            ...
            movie_radio_validation::validate_file(
                &input_media,
                &truth_json,
                &output,
                &cfg,
        ...
        (None, Some(subtitles), None) => {
            ...
            let speech = movie_radio_validation::srt::parse_srt_segments(&srt)?;
            let total = total_ms.context("--total-ms required for subtitle validation")?;
            let truth = movie_radio_validation::timeline_from_speech_segments(
        ...
        (None, None, Some(dataset_manifest)) => {
            ...
            let truth = movie_radio_validation::dataset::build_truth_from_manifest(
                &dataset_manifest,
                &input_media.display().to_string(),
                cfg.sample_rate_hz,
                cfg.frame_ms,
...
pub fn handle_verify_timeline(
    ...
    if save_learning {
        ...
        let mut state = adaptive_thresholds::create_learning_state(20);
        for (i, result) in report.segment_results.iter().enumerate() {
            let was_fp = result.is_suspicious;
            adaptive_thresholds::record_verification_result(
                &mut state,
                i,
                was_fp,
        ...
        adaptive_thresholds::save_learning_state(&state, &state_path)?;
        ...
        let learning_db_conn = rt.block_on(database::LearningDb::new(&db_path))?;
        for (i, result) in report.segment_results.iter().enumerate() {
            let segment = database::VerifiedSegment {
                start_ms: result.start_ms as i64,
                end_ms: result.end_ms as i64,
                confidence: result.original_confidence as f64,
                spectral_features: database::SpectralFeatures {
...
pub fn handle_tag(
    ...
    tags::add_tags(&input_media, &mut timeline, rules.as_ref())?;
...
pub fn handle_prompt(
    ...
    let cfg = config::build_analysis_config(config_path, None, None, None, None, None, None, None)?;
    let mut timeline = read_timeline(&input_json)?;
    prompts::add_prompts(&mut timeline, &cfg);
    write_json_pretty(&output, &timeline)?;
    Ok(())
}
...
pub fn handle_update_thresholds(
    ...
    let recommendations_json = if let Some(db_path) = learning_db {
        ...
        let mut state = adaptive_thresholds::load_learning_state(&learning_state)?;
        adaptive_thresholds::adjust_thresholds_for_fp_rate(&mut state);
        let recommendations = adaptive_thresholds::generate_threshold_recommendations(&state);
        adaptive_thresholds::save_learning_state(&state, &learning_state)?;
        serde_json::to_vec_pretty(&recommendations)?
    };
...
pub fn handle_merge_timeline(
    ...
    let merge_opts =
        merge::load_merge_options(config_path.as_deref(), min_gap_to_merge, merge_strategy)?;
    ...
    let verified_segment_keys: Option<std::collections::HashSet<(u64, u64)>> =
        if let Some(verified_path) = &verified {
            let report: VerificationReport = read_json(verified_path).with_context(|| {
    ...
    let merged =
        merge::merge_nonvoice_segments(&timeline, &merge_opts, verified_segment_keys.as_ref());
...
pub fn handle_export(
    ...
    match format.as_str() {
        "json" => {
            let export_data = movie_radio_io::json::ExportData::from_timeline(
                &timeline,
        ...
        "edl" => {
            let edl_content =
                movie_radio_io::edl::export_edl(&timeline, verified_segments.as_ref());
            std::fs::write(&output, edl_content)?;
            tracing::info!(format = "edl", output = %output.display(), "exported timeline");
        }
        "vtt" => {
            let vtt_content =
                movie_radio_io::vtt::export_vtt(&timeline, verified_segments.as_ref());
...
pub fn handle_ai_voice_extract(
    ...
    let speech_segments: Vec<Segment> = timeline
        .segments
        .into_iter()
        .filter(|s| s.kind == SegmentKind::Speech)
        .collect();
    let ai_voice_output = movie_radio_types::AiVoiceOutput {
        file: timeline.file,
